    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">Raw Material Database</h1>
            <p className="text-muted-foreground mt-1 text-sm font-medium">Manage components, chemical additives, and inventory levels.</p>
          </div>
        </div>
        <button
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                <div className="space-y-2">
                  <button className="w-full text-left px-4 py-2 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors flex items-center justify-between">
                    ...
                    <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{resources.length}</span>
                  </button>
                  <button className="w-full text-left px-4 py-2 rounded-xl text-xs font-bold text-amber-600 hover:bg-amber-50 transition-colors flex items-center justify-between bg-amber-50/30">
                    ...
                    <span className="bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-black">!</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="md:col-span-3">
          {isLoading && !resources.length ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-50">
               ...
               <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading Warehouse Ledger...</p>
            </div>
          ) : (
            <div className="grid gap-4 overflow-hidden">
              {filteredResources.map((resource) => (
                <div key={resource.id} className="group relative rounded-3xl border border-slate-200 bg-white p-1 pr-6 hover:shadow-2xl hover:border-indigo-100 transition-all duration-300">
                  <div className="flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="flex-1 py-4 text-center md:text-left">
                       <div className="flex flex-col md:flex-row md:items-center gap-3">
                          <h2 className="text-xl font-black text-slate-900 tracking-tight">{resource.name}</h2>
                    ...
                    <div className="py-4 px-6 md:border-l border-slate-100 min-w-[140px] text-center md:text-right">
                       <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Availability</span>
                       <div className="text-2xl font-black text-slate-900 tracking-tighter">
                         {resource.current_stock}<span className="text-xs text-slate-400 ml-1 font-bold uppercase">{resource.unit}</span>
                       </div>
                       <div className="w-16 h-1.5 bg-slate-100 rounded-full mt-2 overflow-hidden ml-auto">
                          <div 
                            ...
                            style={{ width: `${Math.min(100, (resource.current_stock / 500) * 100)}%` }}
                          />
                       </div>
                    </div>
          ...
          <div className="w-full max-w-xl rounded-[40px] bg-white p-10 shadow-2xl border border-white/20 animate-in slide-in-from-bottom-8 duration-500">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Chemical Inventory Form</p>
                 </div>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Chemical Name *</label>
                  <input
                    required
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Base Unit</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Raw Feel / Texture</label>
                  <input
                    className="w-full rounded-2xl border border-slate-200 px-5 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold placeholder:text-slate-300"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Natural Color hex</label>
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : editingResource ? 'UPDATE COMPONENT' : 'COMMIT REGISTRATION'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="w-full max-w-lg rounded-[40px] bg-white p-10 shadow-2xl border border-white/20 animate-in slide-in-from-bottom-8 duration-500">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Ordering: {orderingResource.name}</p>
                 </div>
              </div>
              <button 
            ...
            <form onSubmit={handleCreateOrder} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Fulfillment Supplier *</label>
                <select
                  required
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Required Quantity ({orderingResource.unit}) *</label>
                  <input
                    required
                    type="number"
                    step="any"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Estimated Unit Price (Optional)</label>
                  <input
                    type="number"
                    step="any"
                    className="w-full rounded-2xl border border-slate-200 px-5 py-4 text-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold"
                    ...
                    onChange={(e) => setOrderFormData({ ...orderFormData, unit_price: e.target.value })}
                  />
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Procurement Instructions</label>
                <textarea
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <>GENERATE PO & EMAIL <Send className="h-4 w-4" /></>}
                </button>
              </div>
            </form>
          </div>
