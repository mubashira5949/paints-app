...
export default function Formulas() {
  ...
  const getColorPhoto = (colorId: number): string => {
    try {
      return localStorage.getItem(`color_photo_${colorId}`) || ''
    } catch {
      return ''
    }
  }
...
`
  ...
  const shareViaSystem = async () => {
    ...
    try {
      ...
      if (shareTarget.jpegDataUrl && navigator.canShare) {
        ...
        const file = new File([blob], `${shareTarget.color.name.replace(/\s+/g, '_')}.jpg`, {
          type: 'image/jpeg',
        })
  ...
  const handleSaveColor = async (e: React.FormEvent) => {
    ...
    try {
      if (editingColor) {
        const updated = await apiRequest<{ message: string; color: Color }>(
          `/colors/${editingColor.id}`,
          {
            method: 'PUT',
            body: {
              ...colorForm,
  ...
  const handleDeleteColor = async (e: React.MouseEvent, colorId: number) => {
    e.stopPropagation()
    if (!confirm('Are you sure you want to delete this color? It might have active formulas.'))
      return
    try {
  ...
  const handleSaveFormula = async (e: React.FormEvent) => {
    ...
    const validResources = formulaForm.resources.filter(
      (r) => r.resource_id > 0 && r.quantity_required > 0,
    )
  ...
  const handleDeleteFormula = async (formulaId: number) => {
    if (
      !confirm(
        'Are you sure you want to delete this formula? (This will also delete the associated color!)',
      )
    )
      return
    try {
    ...
    setFormulaForm({
      ...
      resources:
        formula.resources.length > 0
          ...
          : [{ resource_id: 0, quantity_required: 0 }],
    })
    setIsFormulaModalOpen(true)
  }
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center gap-0.5 group">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-[9px] text-slate-500 font-mono uppercase truncate">
                        {color.business_code || 'No Code'}
                      </p>
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        {isManager && (
                          <button
                            onClick={(e) => openEditColor(e, color)}
                    ...
                    {color.tags && color.tags.length > 0 && (
                      <div className="flex flex-wrap gap-0.5 mt-0.5">
                        {color.tags.slice(0, 2).map((tag, i) => (
                          <span
                            key={i}
                            className="text-[8px] bg-slate-100 text-slate-500 px-1 rounded"
                          >
                            {tag}
                          </span>
                        ))}
              ...
              <div className="p-5 border-b bg-slate-50 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                  <div
                    ...
                    style={{
                      backgroundColor: selectedColor.color_code || '#cbd5e1',
                    }}
                  />
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">
                      {selectedColor.name} Formulas
                    </h2>
                    <p className="text-xs text-slate-500">
                      {selectedColor.description || 'No description provided.'}
                    </p>
                  </div>
                </div>
                ...
                {formulas.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-slate-400">
                    ...
                    <p className="font-medium text-slate-500">
                      No active formulas found for this color.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-6">
                            ...
                            {isManager ? (
                              <>
                                <button
                                  ...
                                  className="p-1.5 hover:bg-white border border-transparent hover:border-slate-200 rounded text-slate-500 hover:text-blue-600 shadow-sm transition-all text-xs font-bold flex items-center gap-1"
                                >
                                ...
                                <button
                                  ...
                                  className="p-1.5 hover:bg-white border border-transparent hover:border-slate-200 rounded text-slate-500 hover:text-red-600 shadow-sm transition-all"
                                >
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b bg-slate-50/40 text-left">
                                <th className="px-5 py-2.5 font-bold text-slate-500 text-xs uppercase tracking-wider">
                                  Raw Material
                                </th>
                                <th className="px-5 py-2.5 font-bold text-slate-500 text-xs uppercase tracking-wider text-right">
                                  Required Qty
                                </th>
                              </tr>
                            </thead>
            ...
            <form onSubmit={handleSaveColor} className="p-4 space-y-3 overflow-y-auto flex-1">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    className="h-8 w-10 cursor-pointer bg-white border border-slate-300 rounded"
                  ...
                  <input
                    required
                    type="text"
                    className="flex-1 rounded-md border border-slate-300 px-3 py-1.5 text-xs focus:ring-2 focus:ring-blue-500 outline-none uppercase font-mono"
                    value={colorForm.color_code}
                    onChange={(e) => setColorForm({ ...colorForm, color_code: e.target.value })}
                  />
                </div>
              ...
              <div className="space-y-1">
                ...
                <textarea
                  className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-xs focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                  rows={2}
              ...
              <div className="space-y-1.5 border-t pt-2">
                ...
                {colorForm.photo_dataUrl ? (
                  <div className="relative group">
                    ...
                    <span className="absolute bottom-1 left-1 bg-black/50 text-white text-[9px] px-1.5 py-0.5 rounded font-bold">
                      Will export as JPEG
                    </span>
                  </div>
                ) : (
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        reader.onload = (ev) =>
                          setColorForm({
                            ...colorForm,
                            photo_dataUrl: ev.target?.result as string,
                          })
              ...
              <div className="space-y-2 border-t pt-2 mt-1">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-tighter">
                  Product Types
                </label>
                <div className="grid grid-cols-2 gap-2">
                  {productTypes.map((opt) => (
                    <label key={opt.id} className="flex items-center gap-2 cursor-pointer group">
                      ...
                      />
                      <span className="text-xs font-medium text-slate-600 group-hover:text-slate-900 transition-colors uppercase">
                        {opt.name}
                      </span>
                    </label>
                  ))}
                  ...
                  {inkGrades.map((opt) => (
                    <label key={opt.id} className="flex items-center gap-2 cursor-pointer group">
                      <input
                        ...
                        className="h-3.5 w-3.5 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                        checked={
                          colorForm.grade_ids.includes(opt.id) ||
                          (opt.name === 'OPQ/JS' && colorForm.grade_ids.includes(opt.id))
                        }
                        onChange={(e) => {
                          const val = opt.id
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700">
                  Product Tags <span className="font-normal text-slate-400">(comma separated)</span>
                </label>
                <input
                  type="text"
                  className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-xs focus:ring-2 focus:ring-blue-500 outline-none"
                  placeholder="e.g. oil based, series LCS"
                  value={colorForm.tags}
                  onChange={(e) => setColorForm({ ...colorForm, tags: e.target.value })}
                />
              </div>
              <div className="flex gap-2 pt-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setIsColorModalOpen(false)}
                  className="flex-1 rounded-md border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 rounded-md bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-700 transition-colors shadow-sm"
                >
                  {editingColor ? 'Save Changes' : 'Save Color'}
                </button>
              </div>
            </form>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  ...
                  <input
                    required
                    type="text"
                    className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                    ...
                    <div key={index} className="flex items-center gap-3">
                      <div className="flex-1">
                        <select
                          ...
                          onChange={(e) =>
                            updateResourceRow(index, 'resource_id', Number(e.target.value))
                          }
                          className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none appearance-none bg-slate-50"
                        >
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl border relative z-10 overflow-hidden">
            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-5 flex items-center gap-4">
              <div
                className="h-12 w-12 rounded-xl border-2 border-white/20 shadow-lg shrink-0"
                style={{
                  backgroundColor: shareTarget.color.color_code || '#cbd5e1',
                }}
              />
              {shareTarget.jpegDataUrl ? (
                <img
                  src={shareTarget.jpegDataUrl}
                  alt={shareTarget.color.name}
              ...
              <div>
                ...
                <p className="text-slate-400 text-xs font-mono">
                  {shareTarget.color.business_code || shareTarget.color.color_code}
                </p>
              </div>
              <button
                onClick={() => setIsShareModalOpen(false)}
                className="ml-auto text-slate-400 hover:bg-white/20 p-1 rounded-full transition-colors"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100 italic">
                    Auto-copies text
                  </span>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="col-span-2 text-center flex flex-col gap-1">
                      <p className="text-[10px] text-slate-400 font-medium italic mt-1">
                        WhatsApp shares Image + Info
                      </p>
                      <p className="text-[10px] text-slate-400 font-medium italic">
                        Email shares formatted Text template
                      </p>
                    </div>
                  </div>
                ) : (
