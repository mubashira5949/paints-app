    ...
    try {
      await apiRequest('/inventory/sales', {
        ...
        body: {
          ...
          notes:
            notes || `Sale of ${selectedProduct.color_name} (${selectedPackSize}kg x ${quantity})`,
        },
      })
      setSuccess(true)
      setTimeout(() => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Record and update daily paint sales and stock dispatches.
          </p>
        </div>
                ...
                filteredInventory.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4
                          className={`font-bold transition-colors ${selectedProduct?.color_id === item.color_id ? 'text-emerald-900' : 'text-slate-800'}`}
                        >
                          {item.color_name}
                        </h4>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          {item.business_code} • {item.series}
                        </p>
                      </div>
                    </div>
                    {selectedProduct?.color_id === item.color_id ? (
            ...
            <div className="rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-20 flex flex-col items-center justify-center text-center space-y-4 h-full min-h-[500px]">
              ...
              <div>
                ...
                <p className="text-slate-500 max-w-[300px] mt-2 font-medium italic">
                  Select a paint color from the inventory catalog to start recording a dispatch or
                  sale.
                </p>
              </div>
            </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                ...
                <div className="flex items-center gap-6 relative z-10">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      >
                        Stock: {selectedProduct.stock_status}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 min-w-[140px] text-center">
                  ...
                  <p className="text-2xl font-black text-slate-800">
                    {formatUnit(selectedProduct.total_mass_kg, unitPref)}
                  </p>
                </div>
              </div>
                      ...
                      {availablePacks.length === 0 ? (
                        ...
                        availablePacks.map((pack) => (
                          ...
                          >
                            ...
                            <span
                              className={`text-[10px] font-black uppercase tracking-tight ${selectedPackSize === pack.pack_size_kg ? 'text-emerald-100' : 'text-slate-400'}`}
                            >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <div className="relative">
                        ...
                        />
                        <span className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-400 font-bold uppercase tracking-widest text-sm">
                          Units
                        </span>
                      </div>
                      {selectedPackSize && (
                ...
                quantity >
                  ...
                  <div className="rounded-2xl border border-red-200 bg-red-50 p-4 flex items-center gap-3 animate-in shake duration-500">
                    ...
                    <p className="text-xs font-bold text-red-800 uppercase tracking-tight">
                      Warning: Selected quantity exceeds available stock units!
                    </p>
                  </div>
                )}
            </div>
