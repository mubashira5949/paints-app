...
export default function Clients() {
  ...
  const resetForm = () => {
    ...
    setShippingAddresses([]); setAddrLabel(""); setAddrAddress(""); setAddrIsDefault(false);
  };
  ...
  const handleAddAddress = () => {
    ...
    setAddrLabel(""); setAddrAddress(""); setAddrIsDefault(false);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2 font-medium">Onboard and manage your customer accounts.</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between gap-4">
          ...
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap">{filtered.length} clients</p>
        </div>
                  ...
                  <div className="bg-slate-50/50 border-t border-slate-100 px-6 py-5 animate-in slide-in-from-top-2 duration-200">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div>
                        ...
                        {c.billing_address ? (
                          <p className="text-sm text-slate-700 font-medium leading-relaxed">{c.billing_address}</p>
                        ) : (
                      ...
                      <div className="md:col-span-2">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Shipping Addresses</p>
                        {c.shipping_addresses.length === 0 ? (
                          <p className="text-sm text-slate-400 italic">No shipping addresses added</p>
                        ) : (
                            ...
                            {c.shipping_addresses.map(addr => (
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <p className="text-slate-500 mt-0.5 leading-relaxed">{addr.address}</p>
                                </div>
                              </div>
                            ))}
                          </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl relative z-10 flex flex-col max-h-[90vh] border border-slate-200 scale-in-center">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Fill in the customer details below</p>
              </div>
              <button onClick={() => { setIsModalOpen(false); resetForm(); }} className="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-400 hover:text-slate-700">
                <X className="w-5 h-5" />
              </button>
            </div>
                  ...
                  <div className="p-4 rounded-2xl bg-violet-50/50 border border-violet-100 space-y-3">
                    <div className="flex flex-col md:flex-row gap-3 items-end">
                      <div className="w-full md:w-36">
                        ...
                        <input
                          type="text" value={addrLabel} onChange={e => setAddrLabel(e.target.value)}
                          placeholder="e.g. Warehouse A"
                          className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-sm font-medium outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500"
                        />
                      </div>
                      <div className="flex-1">
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 block">Full Address</label>
                        <input
                          type="text" value={addrAddress} onChange={e => setAddrAddress(e.target.value)}
                      ...
                      <div className="flex items-center gap-2 pb-1">
                        <input
                          type="checkbox" id="addr-default" checked={addrIsDefault}
                          onChange={e => setAddrIsDefault(e.target.checked)}
                          className="w-4 h-4 rounded accent-violet-600"
                        />
                        <label htmlFor="addr-default" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap">Default</label>
                      </div>
                      <button
                        type="button" onClick={handleAddAddress}
                    ...
                    {shippingAddresses.length > 0 && (
                      <div className="space-y-1.5 pt-1">
                        {shippingAddresses.map((a, i) => (
                          <div key={i} className="flex items-center justify-between bg-white rounded-xl border border-slate-100 px-3 py-2.5 text-sm shadow-sm">
                            <div className="flex items-center gap-2">
                              <MapPin className="w-3.5 h-3.5 text-violet-600 shrink-0" />
                              <span className="font-bold text-slate-800">{a.label}</span>
                              <span className="text-slate-500 truncate max-w-[200px]">{a.address}</span>
                              {a.isDefault && <span className="text-[9px] bg-violet-600 text-white px-1.5 py-0.5 rounded font-black uppercase">Default</span>}
                            </div>
                            <button type="button" onClick={() => handleRemoveAddress(i)} className="text-red-400 hover:text-red-600 transition-colors text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded hover:bg-red-50">
                              Remove
                            </button>
                          </div>
                        ))}
            ...
            {/* Footer */}
            <div className="p-5 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3 rounded-b-3xl">
              <button type="button" onClick={() => { setIsModalOpen(false); resetForm(); }} className="px-5 py-2.5 text-sm font-bold text-slate-500 uppercase tracking-widest hover:text-slate-800 transition-colors">
                Cancel
              </button>
              <button
