  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto px-4 py-12 print:p-0 print:max-w-none">
      <div className="flex items-center justify-between no-print">
        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Standard Procurement Ledger</h1>
        <div className="px-4 py-2 bg-orange-50 text-orange-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-orange-100 flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-orange-500 animate-pulse" /> Final Confirmation Layout
        </div>
      </div>
      ...
      {isLoading && !orders.length ? (
        <div className="flex flex-col items-center justify-center py-40 gap-6">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Compiling Professional Document...</p>
        </div>
      ) : (
        <div className="space-y-12">
          {orders.map((order) => (
            <div key={order.id} className={`group bg-white overflow-hidden transition-all duration-500 ${expandedId === order.id ? 'no-shadow print:shadow-none' : 'rounded-[40px] border border-slate-100 shadow-xl shadow-slate-100/50 hover:shadow-2xl'}`}>
              ...
              >
                 <div className="flex items-center gap-10">
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Purchase Order</span>
                        <span className="text-2xl font-black text-slate-900">{generateUniquePONumber(order.id, order.created_at)}</span>
                    </div>
                    <div className="h-10 w-px bg-slate-200" />
                    <div className="flex flex-col">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Commercial Partner</span>
                 ...
                 <div className="flex items-center gap-4">
                    ...
                    {expandedId === order.id ? <ChevronUp className="h-6 w-6 text-orange-500" /> : <ChevronDown className="h-6 w-6 text-slate-300" />}
                 </div>
              </div>
                ...
                <div className="relative p-12 lg:p-20 bg-white print:p-0">
                   ...
                   <div className="grid grid-cols-2 gap-20 mb-20">
                      ...
                      <div className="space-y-6">
                         <div className="text-sm font-bold text-slate-500 space-y-1">
                            ...
                            <p className="text-slate-900 font-black flex items-center gap-2 mt-4 uppercase tracking-tighter">GSTIN: {BUYER_INFO.gstin}</p>
                         </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end text-right space-y-6">
                         <div className="space-y-1">
                            ...
                            <p className="text-sm font-bold text-slate-500">{new Date(order.created_at).toLocaleDateString()}</p>
                         </div>
                         <div className="px-4 py-1.5 border-2 border-orange-500 rounded-xl bg-orange-50/50">
                            <span className="text-xs font-black text-orange-600 uppercase tracking-widest">PO Number: {generateUniquePONumber(order.id, order.created_at)}</span>
                         </div>
                         <div className="space-y-1">
                            <span className="text-sm font-black text-slate-900">Delivery Date:</span>
                            <p className="text-sm font-bold text-slate-500">{new Date(new Date(order.created_at).getTime() + 15*24*60*60*1000).toLocaleDateString()}</p>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="grid grid-cols-2 gap-20 mb-16">
                      ...
                      <div className="space-y-6">
                         ...
                         <div className="text-sm font-bold text-slate-500 space-y-1">
                            <p className="text-slate-800">{BUYER_INFO.name}</p>
                            <p className="max-w-[300px] leading-relaxed">{BUYER_INFO.address}</p>
                            <p className="flex items-center gap-2 mt-4 font-black text-slate-900">Contact No.- {BUYER_INFO.phone}</p>
                         </div>
                      </div>
                   </div>
                      ...
                      <table className="w-full border-collapse border-b-2 border-orange-500">
                         <thead>
                            <tr className="bg-white border-t-2 border-orange-500">
                               <th className="px-6 py-4 text-xs font-black text-slate-900 uppercase border-r border-orange-100">Item No.</th>
                               <th className="px-6 py-4 text-xs font-black text-slate-900 uppercase text-left border-r border-orange-100">Item Description</th>
                               <th className="px-6 py-4 text-xs font-black text-slate-900 uppercase text-center border-r border-orange-100">Quantity</th>
                               <th className="px-6 py-4 text-xs font-black text-slate-900 uppercase text-center border-r border-orange-100">Unit Price</th>
                               <th className="px-6 py-4 text-xs font-black text-slate-900 uppercase text-right">Total Price</th>
                            </tr>
                         </thead>
                         <tbody className="divide-y-2 divide-orange-50">
                            {order.items?.map((item, index) => (
                               <tr key={item.id}>
                                  <td className="px-6 py-6 text-center border-r border-orange-100 font-bold text-slate-500">{index + 1}.</td>
                                  <td className="px-6 py-6 border-r border-orange-100">
                                     <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-slate-400 font-bold uppercase no-print cursor-pointer hover:text-orange-500 truncate mt-1" onClick={() => openEditModal(item)}>Edit Unit Quantity</span>
                                     </div>
                                  </td>
                                  <td className="px-6 py-6 text-center border-r border-orange-100 font-bold text-slate-600">{item.quantity}</td>
                                  <td className="px-6 py-6 text-center border-r border-orange-100 font-bold text-slate-600">₹{item.unit_price.toFixed(2)}</td>
                                  <td className="px-6 py-6 text-right font-black text-slate-900 tracking-tight">₹{(item.quantity * item.unit_price).toFixed(2)}</td>
                               </tr>
                            ))}
                         </tbody>
                      </table>
                      ...
                      <div className="w-full max-w-[320px] rounded-2xl bg-orange-50/50 border-2 border-orange-100 overflow-hidden shadow-2xl shadow-orange-100/50">
                          <div className="p-6 space-y-4 font-bold text-sm">
                             ...
                             <div className="flex justify-between items-center pt-2">
                                ...
                                <span className="text-2xl font-black text-orange-500 tracking-tighter">₹{(calculateSubtotal(order.items) + calculateTax(calculateSubtotal(order.items))).toFixed(2)}</span>
                             </div>
                          </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center justify-between pt-10 border-t-2 border-slate-100">
                      <div className="flex flex-col gap-1">
                          ...
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-7">System Automated Signature</p>
                      </div>
                   ...
                   <div className="mt-20 no-print flex items-center justify-between bg-slate-50 p-6 rounded-[32px] border border-slate-200">
                      ...
                      <div className="flex items-center gap-3">
                         <button onClick={() => window.print()} className="p-3 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-orange-500 transition-all shadow-sm">
                             <Printer className="h-5 w-5" />
                         </button>
                         <div className="h-8 w-px bg-slate-200" />
                         <button onClick={() => { navigator.clipboard.writeText(window.location.host + "/po/" + order.share_token); alert("PO URL Copied!"); }} className="p-3 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-blue-500 transition-all shadow-sm">
                             <LinkIcon className="h-5 w-5" />
                         </button>
                         <button onClick={() => window.location.href = `mailto:${order.supplier_email}`} className="px-6 py-3 rounded-2xl bg-blue-50 text-blue-600 text-[11px] font-black uppercase tracking-widest hover:bg-blue-100 transition-all">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{selectedItem.resource_name}</p>
                    </div>
                </div>
                <button onClick={() => setIsEditModalOpen(false)} className="p-2 rounded-xl text-slate-400"><XCircle className="h-6 w-6" /></button>
              </div>

              <form onSubmit={handleUpdateItem} className="space-y-6">
                 <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">New Quantity ({selectedItem.unit})</label>
                    <input 
                        required
                        type="number"
