...
export default function ProductionPackaging() {
  ...
  const alreadyPackaged =
    run?.packaging?.reduce((sum, p) => sum + toDisplayValue(Number(p.volume_kg), unitPref), 0) ?? 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isOverAllocated) {
      setSubmitErr(
        `Total allocation (${totalAllocated.toFixed(1)}${unitPref}) exceeds batch size (${displayBatchVolume}${unitPref}).`,
      )
      return
    }
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-10">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          {run.color_name} · {run.formula_name} · {displayBatchVolume.toLocaleString()}
          {unitPref} available
        </p>
      </div>
        ...
        <div className="flex items-center justify-between text-sm">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider mt-0.5">
              Progress to total batch
            </p>
          </div>
          <div className="text-right">
            <span
              className={`text-lg font-bold font-mono ${isOverAllocated ? 'text-red-600' : 'text-slate-700'}`}
            >
              {totalAllocated.toFixed(1)} / {displayBatchVolume.toLocaleString()}
              {unitPref}
            </span>
        ...
        <div className="space-y-1.5">
          <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden flex shadow-inner">
            ...
            <div
              ...
              style={{
                width: `${(alreadyPackaged / displayBatchVolume) * 100}%`,
              }}
            ...
            <div
              ...
              style={{
                width: `${(currentAllocated / displayBatchVolume) * 100}%`,
              }}
          ...
          <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest">
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1 text-purple-600">
                <span className="w-2 h-2 rounded-full bg-purple-400" /> {alreadyPackaged.toFixed(1)}{' '}
                Done
              </span>
              <span className="flex items-center gap-1 text-blue-600">
                <span className="w-2 h-2 rounded-full bg-blue-500" /> +{currentAllocated.toFixed(1)}{' '}
                New
              </span>
            </div>
          ...
          <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
            <div className="p-4 border-b bg-slate-50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                {remaining > 0 ? (
                  ...
                  >
                    Pack Remaining ({remaining.toFixed(1)}
                    {unitPref})
                  </button>
                ) : (
            ...
            <div className="divide-y">
              {rows.map((row, idx) => {
                ...
                const rowVolume =
                  !isNaN(size) && !isNaN(qty) && size > 0 && qty > 0 ? size * qty : null
