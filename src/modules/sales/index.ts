...
export default async function (fastifyRaw: FastifyInstance) {
    ...
    /**
     * GET /sales/reports/hsn-wise
     * Exports an HSN-wise sales report as CSV.
     */
    fastify.get('/reports/hsn-wise', {
            ...
            try {
                ...
                const result = await client.query(`
                    SELECT 
                        ...
                        SUM(t.quantity_kg) as total_weight_kg
                    FROM finished_stock_transactions t
                    JOIN colors c ON t.color_id = c.id
                    WHERE t.transaction_type = 'sale'
                ...
                result.rows.forEach(row => {
                    const productName = row.product_name.includes(',') ? `"${row.product_name}"` : row.product_name
